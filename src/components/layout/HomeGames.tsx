    ...
    <div >
       <div className="pt-2 px-5 styled-scroll overflow-x-scroll flex justify-between lg:justify-center items-center bg-primary-radial bg-primary-radial-alt gap-8 lg:gap-16">
        {gameCategories.map((item)=>{
            return <div key={item.id} className={item.id===0 ? ' border-b-4 border-secondary pb-1' :'pb-2'}>
     ...
     <div className="flex flex-wrap ">
        {gameCategories.splice(1,6).map((item)=>{
            return <div key={item.id} className='cursor-pointer basis-1/2 md:basis-1/3 lg:basis-1/4 xl:basis-1/6 px-5 py-3 relative group'>
                <h1 className='uppercase font-bold text-secondaryGradient'>{item.name}</h1>
                <img src="https://mogame-bucket.s3.ap-southeast-1.amazonaws.com/GameFile/MakerProjectFile/JILI/102/icon.png" className='w-full h-auto rounded-sm group-hover:scale-105 transition-all duration-300 ease-in-out' />
